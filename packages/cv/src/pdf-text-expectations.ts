...
/**
 * Substrings that must appear in the CV PDF text extract when {@link CvData} is rendered.
 * Keeps PDF tests aligned with the same source as `lib/cv` (no checked-in golden snapshots).
 */
export function buildCvPdfRequiredSubstrings(data: CvData): readonly string[] {
  const parts: string[] = [];
  ...
  for (const project of data.personalProjects) {
    ...
    // Project link shows the raw href; match hostname and stable query ids for PDF text checks.
