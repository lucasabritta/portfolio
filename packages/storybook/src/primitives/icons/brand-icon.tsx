/**
 * Monochrome brand marks. SVG path data from Simple Icons v11.15.0 (CC0-1.0) —
 * https://simpleicons.org/
 */
import clsx from "clsx";

import type { BrandIconName, BrandIconSize } from "./brand-icon-types";
import styles from "./brand-icon.module.css";

export type BrandIconProps = {
  name: BrandIconName;
  /** Default `md` matches action buttons; use `sm` in dense inline rows (footer, contact). */
  size?: BrandIconSize;
  className?: string;
};

const PATH_BY_NAME: Record<BrandIconName, string> = {
  github:
...
export function BrandIcon({ name, size = "md", className }: BrandIconProps) {
  return (
    <svg
      className={clsx(styles.icon, size === "sm" ? styles.sizeSm : null, className)}
      viewBox="0 0 24 24"
