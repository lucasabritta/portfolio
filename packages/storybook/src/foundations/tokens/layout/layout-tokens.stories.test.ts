...
export const layoutTokensPlay: StoryPlayFn = async ({ canvasElement }) => {
  ...
  expect(canvas.getByText("Compact inline brand icons (footer, contact)")).toBeVisible();
  expect(canvas.getByText("Default icon size in buttons and CTAs")).toBeVisible();
