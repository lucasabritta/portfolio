...
export const summarySectionDefaultPlay: StoryPlayFn = async ({ canvasElement }) => {
  ...
  const firstHighlight = cvData.summaryHighlights[0];
  if (firstHighlight) {
    expect(canvas.getByText(firstHighlight)).toBeVisible();
  }
  const firstTech = cvData.techStack[0];
  if (firstTech) {
    expect(canvas.getByText(firstTech)).toBeVisible();
  }
};
...
export const summarySectionLongContentPlay: StoryPlayFn = async ({ canvasElement }) => {
  ...
  const firstHighlight = cvData.summaryHighlights[0];
  if (firstHighlight) {
    expect(canvas.getByText(new RegExp(firstHighlight.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")))).toBeVisible();
  }
