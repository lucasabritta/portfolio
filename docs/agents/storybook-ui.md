...
Use **Node.js 24.15.0** (see `.nvmrc`, package `engines`, and the Docker image pin) so Storybook 10 and Vitest match CI. Storybook’s CLI enforces **20.19+ or 22.12+**.
...
The **`Dockerfile`** uses **`public.ecr.aws/docker/library/node:24.15.0-bookworm-slim`**. The **`development`** stage builds **`FROM deps`** so Playwright system libraries are present for **`yarn test:storybook`**.
