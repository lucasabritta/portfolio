...
Stack file: **`docker-compose.yml`** at the repo root. The **`frontend`** image bakes in **Yarn 1.22.22** via `corepack prepare` in the `Dockerfile`, so `docker compose run … yarn` does not prompt to download Yarn.

## Layout source of truth

React-PDF uses **points**, not browser px. Table geometry is aligned to the DOCX `tblGrid` (3285 + 8235 twips) via `apps/frontend/lib/cv-pdf/constants.ts` and `apps/frontend/lib/cv-pdf/styles/layout.ts`.

**Sidebar URLs:** Do **not** set `overflow: 'hidden'` on the whole left column — it clips multi-line links. Long Play Store URLs use a fixed inner width (`CV_PDF_LEFT_INNER_WIDTH_PT`), wrapping **`Text` inside `Link`** (`apps/frontend/lib/cv-pdf/sections/left-column/projects.tsx`), and **`formatSidebarUrlForPdf`** (`apps/frontend/lib/cv-pdf/sections/text-format.ts`) so text runs stay within the column.
...
Writes `tmp-cv-compare/docker-latest-cv.pdf` at the repository root (bind-mounted from the container as `/workspace/tmp-cv-compare/…`).
...
With **`docker compose up frontend`**, you can also download the PDF from the running app at **`/api/cv`**.
...
docker compose --profile cv run --rm -v "$HOME/Downloads:/docs:ro" cv-tools \
  python scripts/compare_cv_pdf_pages.py /docs/Lucas_Abritta_EM.pdf /workspace/tmp-cv-compare/docker-latest-cv.pdf
```
...
docker compose --profile cv run --rm cv-tools \
  python scripts/cv/preview_pdf_page.py /workspace/tmp-cv-compare/docker-latest-cv.pdf /workspace/tmp-cv-compare/page1.png
```
...
Regenerate bundled TTFs under `apps/frontend/public/cv-fonts/`:
