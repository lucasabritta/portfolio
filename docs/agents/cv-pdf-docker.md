...
Stack file: **`docker-compose.yml`** at the repo root. The **`frontend`** image bakes in **Yarn 1.22.22** via `corepack prepare` in the `Dockerfile`, so `yarn cv:dump:docker` / `docker compose run … yarn` does not prompt to download Yarn.

## Layout source of truth

React-PDF uses **points**, not browser px. Table geometry is aligned to the DOCX `tblGrid` (3285 + 8235 twips) via `apps/backend/src/cv-pdf/constants.ts` and `styles/layout.ts`.

**Sidebar URLs:** Do **not** set `overflow: 'hidden'` on the whole left column — it clips multi-line links. Long Play Store URLs use a fixed inner width (`CV_PDF_LEFT_INNER_WIDTH_PT`), wrapping **`Text` inside `Link`** (`apps/backend/src/cv-pdf/sections/left-column/projects.tsx`), and **`formatSidebarUrlForPdf`** (`apps/backend/src/cv-pdf/sections/text-format.ts`) so pdf.js does not emit a single text run wider than the column. **`apps/backend/src/cv-pdf/cv-pdf-integrity.test.ts`** checks both text fragments and **geometry** (`x + width` vs `CV_PDF_RIGHT_COLUMN_INNER_LEFT_PT`) using **`apps/backend/src/cv-pdf/cv-pdf-pdfjs.ts`** (bundled pdf.js).
...
docker compose run --rm \
  ...
  frontend yarn vitest run --project unit-backend apps/backend/src/cv-pdf/cv-pdf-dump.test.ts
```
