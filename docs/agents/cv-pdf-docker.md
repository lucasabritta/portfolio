...
React-PDF uses **points**, not browser px. Table geometry is aligned to the DOCX `tblGrid` (3285 + 8235 twips) via `components/cv-pdf/constants.ts` and `styles/layout.ts`.

**Sidebar URLs:** Do **not** set `overflow: 'hidden'` on the whole left column — it clips multi-line links. Long Play Store URLs use a fixed inner width (`CV_PDF_LEFT_INNER_WIDTH_PT`), wrapping **`Text` inside `Link`** (`sections/left-column/projects.tsx`), and **`formatSidebarUrlForPdf`** (`sections/text-format.ts`) so pdf.js does not emit a single text run wider than the column. **`components/cv-pdf/cv-pdf-integrity.test.ts`** checks both text fragments and **geometry** (`x + width` vs `CV_PDF_RIGHT_COLUMN_INNER_LEFT_PT`) using **`components/cv-pdf/cv-pdf-pdfjs.ts`** (bundled pdf.js).
