...
import { CV_PDF_RIGHT_COLUMN_INNER_LEFT_PT } from "@cv-pdf/constants";
import {
  flattenCvPdfTextForSubstringMatch,
  normalizeCvPdfExtractedText,
} from "@cv-pdf/pdf-text-normalize";
import { extractPdfPageTextItems, pdfParse } from "@cv-pdf/cv-pdf-pdfjs";
import { renderCvPdfToBuffer } from "@cv-pdf/cv-pdf-vitest-helpers";
import { resumeData } from "@portfolio/resume-content";

import { buildCvPdfRequiredSubstrings } from "../pdf-text-expectations";

describe("CV PDF integrity (data from lib/cv)", () => {
  it("text extract contains every required substring derived from resumeData", async () => {
    ...
    for (const fragment of buildCvPdfRequiredSubstrings(resumeData)) {
  ...
  it("keeps Play Store URL text runs inside the left column (no bleed into work history)", async () => {
    const href = resumeData.personalProjects[0]?.href ?? "";
